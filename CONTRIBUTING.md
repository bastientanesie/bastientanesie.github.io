...
A Post or a Project is a folder in `src/content/` whose name (kebab-case) is its `id`. Schemas are validated at build time. Content used only by the end-to-end tests lives in `tests/fixtures/content/`, with the same layout; `make test` builds the site from it. Credit any third-party visual, tool or library in `src/data/credits.ts`.
